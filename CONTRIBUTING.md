...
Always review autogenerated migrations before committing (CHECK constraints, defaults, PK sequences). CI runs ruff, `alembic upgrade head`, seed, pytest (Postgres 16 service container), and `npm run build` in `frontend/` (see `.github/workflows/ci.yml`).
